...
<style>
  ...
  .logo-wrapper {
    ...
    /* Enable interactions so the hover effect works while passing mouse events to canvas */
    pointer-events: auto;
  }

  /* Fade-in Animation & Sizing for Logo */
  .logo-overlay {
    max-width: 620px; /* Increased max width for a larger presence */
    width: 85vw;      /* Scales nicely on responsive screens */
    opacity: 0;
    cursor: pointer;
    
    /* Transition for the smooth hover scaling */
  ...
  /* Hover effect to scale and enlarge logo on mouse over */
  .logo-overlay:hover {
    transform: scale(1.08); /* Scales up by 8% on hover */
  }
